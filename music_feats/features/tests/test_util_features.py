...
class TestRMS:
    ...
    def test_scale(self):
        for _ in range(10):
            ...
            npt.assert_allclose(val, scalar, rtol=10**-3)
            val = extractor.rms(sawsig*scalar, decomposition=False)
            npt.assert_allclose(val, scalar/np.sqrt(3), rtol=10**-3)
    ...
    def test_sawtooth(self):
        ...
        npt.assert_allclose(val, 1/np.sqrt(3), rtol=10**-3)

    def test_sine(self):
        val = extractor.rms(sinsig, decomposition=False)
        npt.assert_allclose(val, 1/np.sqrt(2), rtol=10**-3)
    ...
    def test_sine_windows(self):
        ...
        npt.assert_allclose(val, 1/np.sqrt(2)*np.ones(46), rtol=10**-3)
...
class TestZCR:
    ...
    def test_alt(self):
        ...
        npt.assert_allclose(val, 1, rtol=10**-3)
